...
<head>
    <meta charset="UTF-8">
    <title>Edit Candidate | CampusHub</title>
...
<body>
    ...
    <div class="container">
        <div class="header">
            <h3><i class="fas fa-users"></i> Edit Candidate</h3>
        </div>

        <div class="card">
            <div class="card-body">
                <form method="POST" action="{{ route('manager.candidates.update', $candidate->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                        ...
                        <select id="studentSelect" class="form-select" onchange="fillStudentInfo()">
                            ...
                            @foreach ($students as $student)
                                <option 
                                    ...
                                    {{ $student->registration_number == $candidate->student_regno ? 'selected' : '' }}
                                >
                    ...
                    <div id="student-info" class="student-info {{ $candidate->student_regno ? '' : 'd-none' }}">
                        <div><strong>Name:</strong> <span id="student-name">{{ $candidate->student->name ?? '' }}</span></div>
                        <div><strong>Course:</strong> <span id="student-course">{{ $candidate->student->course->name ?? '' }}</span></div>
                        <div><strong>Faculty:</strong> <span id="student-faculty">{{ $candidate->student->course->faculty->name ?? '' }}</span></div>
                    </div>

                    <input type="hidden" id="student_regno" name="student_regno" value="{{ $candidate->student_regno }}">
                    <input type="hidden" id="course_id" name="course_id" value="{{ $candidate->course_id }}">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="policies" id="policies" class="form-control" rows="4" required>{{ $candidate->policies }}</textarea>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        @if($candidate->image)
                            <img src="{{ asset('storage/' . $candidate->image) }}" alt="Candidate Image" class="image-preview" width="100">
                        @endif
                    </div>

                    <div class="button-group">
                        <a href="{{ route('manager.candidates.index') }}" class="btn btn-secondary">
    ...
    <script>
        ...
        function fillStudentInfo() {
            ...
            if (selectedOption && selectedOption.value) {
                ...
                document.getElementById('student_regno').value = '';
                document.getElementById('course_id').value = '';
            }
        }

        // Initialize student info on page load
