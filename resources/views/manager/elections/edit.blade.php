...
<head>
    <meta charset="UTF-8">
    <title>Edit Election | CampusHub</title>
...
<body>
    ...
    <div class="container">
        <div class="header">
            <h3><i class="fas fa-bullhorn"></i> Edit Election</h3>
        </div>

        <div class="card">
            <div class="card-body">
                <form action="{{ route('manager.elections.update', $election->id) }}" method="POST">
                    @csrf
                    @method('PUT')

                    <div class="form-group">
                        <label for="title" class="form-label">Election Title</label>
                        <input type="text" class="form-control" id="title" name="title" required value="{{ $election->title }}">
                    </div>

                    <div class="form-group">
                        <label for="category" class="form-label">Category</label>
                        <select class="form-select" id="category" name="category" required>
                            <option value="president" {{ $election->category == 'president' ? 'selected' : '' }}>President</option>
                            <option value="minister" {{ $election->category == 'minister' ? 'selected' : '' }}>Minister</option>
                            <option value="class_representative" {{ $election->category == 'class_representative' ? 'selected' : '' }}>Class Representative</option>
                            <option value="other" {{ $election->category == 'other' ? 'selected' : '' }}>Other</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select" id="scope" name="scope" required>
                            <option value="university" {{ $election->scope == 'university' ? 'selected' : '' }}>University-wide</option>
                            <option value="faculty" {{ $election->scope == 'faculty' ? 'selected' : '' }}>Faculty</option>
                            <option value="course" {{ $election->scope == 'course' ? 'selected' : '' }}>Course</option>
                        </select>
                    </div>

                    <div class="form-group {{ $election->scope != 'faculty' && $election->scope != 'course' ? 'd-none' : '' }}" id="faculty-group">
                        <label for="faculty_id" class="form-label">Select Faculty</label>
                        <select class="form-select" id="faculty_id" name="faculty_id">
                            <option value="">Select Faculty</option>
                            @foreach ($faculties as $faculty)
                                <option value="{{ $faculty->id }}" {{ $election->faculty_id == $faculty->id ? 'selected' : '' }}>
                                    {{ $faculty->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group {{ $election->scope != 'course' ? 'd-none' : '' }}" id="course-group">
                        <label for="course_id" class="form-label">Select Course</label>
                        <select class="form-select" id="course_id" name="course_id">
                            <option value="">Select Course</option>
                            @foreach ($courses as $course)
                                <option value="{{ $course->id }}" data-faculty="{{ $course->faculty_id }}" {{ $election->course_id == $course->id ? 'selected' : '' }}>
                                    {{ $course->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" class="form-control" id="start_date" name="start_date" required value="{{ $election->start_date }}">
                    </div>

                    <div class="form-group">
                        <label for="end_date" class="form-label">End Date</label>
                        <input type="date" class="form-control" id="end_date" name="end_date" required value="{{ $election->end_date }}">
                    </div>

                    <div class="button-group">
                        <a href="{{ route('manager.elections.index') }}" class="btn btn-secondary">
                            <i class="fas fa-arrow-left"></i> Back
                        </a>
                        <button type="submit" class="btn btn-warning text-white">
                            <i class="fas fa-sync-alt"></i> Update Election
                        </button>
                    </div>
                </form>
            </div>
    ...
    <script>
        ...
        // Form logic for scope and faculty/course filtering
        ...
        scopeSelect.addEventListener('change', function () {
            facultyGroup.classList.add('d-none');
            courseGroup.classList.add('d-none');

            if (this.value === 'faculty') {
                facultyGroup.classList.remove('d-none');
        ...
        facultySelect.addEventListener('change', function () {
            ...
            for (const option of courseSelect.options) {
                ...
                option.style.display = (option.dataset.faculty === selectedFacultyId) ? '' : 'none';
            }
            courseSelect.value = '';
        });
