...
<head>
    <meta charset="UTF-8">
    <title>Registered Students | CampusHub Admin</title>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .btn-warning { background: rgba(243,156,18,0.1); color: #f39c12; border: none; }
        .btn-danger { background: rgba(231,76,60,0.1); color: var(--danger); border: none; }
...
<body>
    ...
    <div class="main-content">
        ...
        <!-- ULTRA-PROFESSIONAL EXCEL IMPORT CARD -->
        <div class="card" style="border-left: 5px solid var(--success);">
            <div class="d-flex align-items-center justify-content-between mb-3">
                <h5 class="mb-0">
                    Import Students from Excel
                </h5>
                <div style="background: rgba(46,204,113,0.1); color: var(--success); width: 40px; height: 40px; border-radius: 12px; display: flex; align-items: center; justify-content: center;">
                    <i class="fas fa-file-excel fa-lg"></i>
                </div>
            </div>
            
            <p class="text-muted small mb-4">
                Upload an Excel file (.xlsx or .xls) with columns: <code>name</code>, <code>registration_number</code>, <code>course_id</code> or <code>course_name</code>.
            </p>

            <form action="{{ route('admin.students.import') }}" method="POST" enctype="multipart/form-data" class="d-flex gap-3 align-items-end flex-wrap">
                @csrf
                
                <div class="flex-grow-1" style="min-width: 280px;">
                    <label class="form-label fw-semibold text-dark">Select Excel File</label>
                    <input type="file" name="file" accept=".xlsx,.xls" class="form-control form-control-lg" required 
                           style="border-radius: 14px; padding: 14px 16px; border: 2px dashed var(--gray-300); background: rgba(255,255,255,0.7);">
                    <div class="form-text mt-2">
                        Supported formats: <strong>.xlsx</strong>, <strong>.xls</strong> • Max size: 10MB
                    </div>
                </div>

                <div>
                    <button type="submit" class="btn btn-success btn-lg d-flex align-items-center gap-2 shadow-sm"
                            style="padding: 14px 32px; border-radius: 16px; font-weight: 600;">
                        Import Students
                    </button>
                </div>
            </form>
                    ...
                    <tbody>
                        @forelse ($students as $student)
                            <tr>
                                ...
                                <td>{{ $student->course->faculty->name ?? 'N/A' }}</td>
                                ...
                                <td class="text-center">
                                    ...
                                    <form action="{{ route('admin.students.destroy', $student) }}" method="POST" class="d-inline"
                                          onsubmit="return confirm('Delete {{ $student->name }} permanently?');">
                                        @csrf
                                        @method('DELETE')
